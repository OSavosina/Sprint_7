from helpers.api_helpers import get_orders_list
...
class TestLoginUser:
    ...
    def test_get_orders_list_success(self):
        response = get_orders_list(RoutesName.ALL_ORDERS)
    ...
    def test_get_orders_list_courier_id_not_found(self):
        response = get_orders_list(RoutesName.COURIER_NOT_FOUND_ID)
    ...
    def test_get_order_near_station_success(self):
        response = get_orders_list(RoutesName.NEAREST_STATIONS_ORDERS)
        assert response.status_code == 200

    @allure.title('Получение списка из 10 заказов')
    def test_get_ten_orders_success(self):
        response = get_orders_list(RoutesName.TEN_ORDERS)
        assert response.status_code == 200
    ...
    def test_get_ten_orders_near_kaluzkaya_station_success(self):
        response = get_orders_list(RoutesName.TEN_ORDERS_NEAREST_STATION_KALUZKAYA)
        assert response.status_code == 200